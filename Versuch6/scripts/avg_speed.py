...
def get_settings():
    ...
    general_sets.x_label = r"$X = D^2 \:(m^2)$"
    general_sets.y_label = r"$y = \langle v \rangle \: (m/s)$"
...
def plot_line(sets):
    ...
    sets.graph_label[1] = r"$y= kX + v$" + r"$\quad mit\quad y= \langle v \rangle \ und\ X=D^2$" +"\n"+ r"$k={}\:$".format(a_str) + r"$(ms)^{-1}\ und\ $" + r"$v={}\: m/s$".format(v_str)
    ...
    plt.subplots_adjust(left=0.162, right = 0.975, bottom = 0.15, top = 0.99)
