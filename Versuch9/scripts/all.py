...
def get_data():
    ...
    df1 = pd.read_excel(xls, sheet_name = 'Wasserwert')
    df2 = pd.read_excel(xls, sheet_name = 'Verdampfungswarme')
    df3 = pd.read_excel(xls, sheet_name = 'Schmelzwarme')
    return {'Wasser': df1, 'Dampf': df2, 'Eis': df3}

def wasser(dfs):
    time = [float(a) for a in dfs['Wasser']['time'][1:]]
    time_err = [float(a)/60 for a in dfs['Wasser']['time_err'][1:]] #convert time from min to sec
    temp = [float(a) for a in dfs['Wasser']['temp'][1:]]
    temp_err = [float(a) for a in dfs['Wasser']['temp_err'][1:]]
    return time, time_err, temp, temp_err

def dampf(dfs):
    time = [float(a) for a in dfs['Dampf']['time'][1:]]
    time_err = [float(a)/60 for a in dfs['Dampf']['time_err'][1:]] #convert time from min to sec
    temp = [float(a) for a in dfs['Dampf']['temp'][1:]]
    temp_err = [float(a) for a in dfs['Dampf']['temp_err'][1:]]
    return time, time_err, temp, temp_err
